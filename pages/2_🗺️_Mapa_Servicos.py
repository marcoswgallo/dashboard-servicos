...
with st.container():
    ...
    if df is not None:
        with col2:
            status_options = sorted(df['STATUS'].unique()) if not df.empty else []
            status = st.multiselect(
                "🔄 Status:",
                options=status_options,
                default=status_options[:1] if status_options else [],
        ...
        if len(df_map) > 0:
            ...
            df_map['TIPO_BASE'] = 'Outros'
            for tipo, bases in base_tipos.items():
                df_map.loc[df_map['BASE'].isin(bases), 'TIPO_BASE'] = tipo
